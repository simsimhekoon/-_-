...
class ClassficationUtil:
    ...
    def drop(self, col):
        # 불필요한 열(ID) 제거
        self.drop(col, axis=1, inplace=True)  # ID라는 컬럼(열)을 삭제하라는 의미

        # 불필요한 Id 컬럼 삭제
        # axis=1 : 컬럼을 의미
        # inplace=True : 삭제한 후 데이터 프레임에 반영하라는 의미

    # myplot(self, 'Height', 'Weight', 'Sex')
    def myplot(self, x_col, y_col, color_field):
        cl = self[color_field].unique()
        col = ['orange', 'blue', 'red', 'yellow', 'black', 'brown', 'green', 'pink', 'violet']

        fig = self[self[color_field] == cl[0]].plot(kind='scatter', x=x_col, y=y_col, color=col[0], label=cl[0])
        for i in range(len(cl)-1):
            self[self[color_field] == cl[i+1]].plot(kind='scatter', x=x_col, y=y_col, color=col[i+1], label=cl[i+1], ax=fig)
    ...
    # myhist(df)
    def myhist(self):
        self.hist(edgecolor='black', linewidth=1.2)
        fig = plt.gcf()
    ...
    def myviolinplot(self, a, b):
        ...
        plt.subplot(1,1,1)
        sns.violinplot(x=a,y=b,data=self)
        plt.show()
    ...
    def data_nanum(self):
        a, b = train_test_split(self, test_size=0.3)
